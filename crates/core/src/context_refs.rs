...
/// Registry for tracking multiple context references.
#[derive(Debug, Clone)]
pub struct ContextRefRegistry {
    pub references: Vec<ContextReference>,
}

impl ContextRefRegistry {
    pub fn new() -> Self {
        Self { references: Vec::new() }
    }

    /// Discover and register context files from a directory.
    pub fn discover_from_dir(&mut self, dir: &Path) {
        for ref_ in discover_context_files(dir) {
            self.references.push(ref_);
        }
    }

    /// Check if any registered context file has changed.
    pub fn has_changed(&self) -> bool {
        self.references.iter().any(|r| r.is_modified())
    }

    /// Invalidate (remove) references that no longer exist.
    pub fn invalidate(&mut self) -> usize {
        let before = self.references.len();
        self.references.retain(|r| r.exists);
        before - self.references.len()
    }

    /// Build a formatted prompt section from all context files.
    pub fn build_prompt_section(&self) -> String {
        let sections: Vec<String> = self.references.iter().filter_map(|r| {
            let path_str = r.path.display();
            match std::fs::read_to_string(&r.path) {
                Ok(content) if !content.is_empty() => {
                    Some(format!("## {} ({})\n```\n{}\n```", r.name, path_str, content))
                }
                _ => None,
            }
        }).collect();
    ...
    /// Register a single context file by path.
    pub fn register(&mut self, path: &Path) {
        if path.exists() {
            self.references.push(ContextReference::new(path.to_path_buf()));
        }
    }
}

impl Default for ContextRefRegistry {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    #[test]
    fn test_known_context_files_list() {
        assert!(KNOWN_CONTEXT_FILES.contains(&"AGENTS.md"));
        assert!(KNOWN_CONTEXT_FILES.contains(&".cursorrules"));
        assert!(KNOWN_CONTEXT_FILES.contains(&"SOUL.md"));
    }

    #[test]
    fn test_registry_new_empty() {
        let registry = ContextRefRegistry::new();
        assert!(registry.references.is_empty());
    }

    #[test]
    fn test_registry_default() {
        let registry = ContextRefRegistry::default();
        assert!(registry.references.is_empty());
    }

    #[test]
    fn test_registry_register_nonexistent() {
        let mut registry = ContextRefRegistry::new();
        registry.register(Path::new("/nonexistent/file.md"));
        assert!(registry.references.is_empty());
    }

    #[test]
    fn test_registry_has_changed_empty() {
        let registry = ContextRefRegistry::new();
        assert!(!registry.has_changed());
    }

    #[test]
    fn test_registry_invalidate_empty() {
        let mut registry = ContextRefRegistry::new();
        let removed = registry.invalidate();
        assert_eq!(removed, 0);
    }
    ...
    fn test_build_prompt_section_empty() {
        let registry = ContextRefRegistry::new();
        let section = registry.build_prompt_section();
